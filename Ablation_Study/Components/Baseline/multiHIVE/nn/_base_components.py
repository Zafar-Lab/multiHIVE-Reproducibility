...
class Encoder(nn.Module):
    ...
    def forward(
        ...
        *cat_list: int,
        ...
        untran_z2 = delta_mu_2 # q_z2.rsample()
        z2 = untran_z2
        ...
        z1_mu = mu_1 # delta_mu_1 + 
        z1_var = torch.exp(0.5 * (logvar_1)) + 1e-4 # delta_logvar_1 +
        ...
        # ).sum(-1)
        KL_z_1 = 0.5 * (z1_mu**2 + torch.exp(z1_var) - z1_var - 1).sum(-1)
        ...
        return q_z1, latent, untran_latent, libsize_acc,#, q_z1r, q_z1p, qz1a,   q_z2,
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.px_scale_decoder = FCLayers(
            n_in=n_hidden + n_shared_latent, # + n_input 
        ...
        if n_output_regions > 0:
            self.z_decoder_accessibility = DecoderPeakVI(
                n_input=n_shared_latent, # n_input + 
    ...
    def forward(
        ...
        *cat_list: int,
        ...
        px = self.px_decoder(torch.cat([z], dim=-1), *cat_list) # , zr
        px_cat_z = torch.cat([px, z], dim=-1) # , zr
        ...
        px_["dropout"] = self.px_dropout_decoder_gene(
            torch.cat([p_mixing_cat_z], dim=-1), *cat_list # , zr
        )
        ...
        if self.n_output_regions > 0:
            ...
            pa = self.z_decoder_accessibility(torch.cat([z], dim=-1), *cat_list) # , za
            pa_["pa"] = pa
