...
class multiHIVEvae(BaseModuleClass):
    ...
    def get_mutual_information_loss(self, z1, z2):
        """Computes the mutual information loss between z1 and z2."""
        # Reshape if z1 has 3 dimensions (n_samples, batch_size, n_latent)
        if z1.dim() == 3:
        ...
        z1_mean = z1 - z1.mean(dim=0, keepdim=True)
        z2_mean = z2 - z2.mean(dim=0, keepdim=True)

        # Compute covariance and variances per dimension over the batch
        cov = (z1_mean * z2_mean).mean(dim=0)
        var1 = (z1_mean ** 2).mean(dim=0)
        var2 = (z2_mean ** 2).mean(dim=0)

        # Pearson correlation coefficient per dimension
        corr = cov / (torch.sqrt(var1 * var2) + 1e-8)
        corr = torch.clamp(corr, -0.999, 0.999)

        # MI = -0.5 * sum(log(1 - corr^2))
        mi = -0.5 * torch.log(1 - corr ** 2)
        return mi.sum()
    
    def get_decorrelation_loss(self, z1, z2):
        """Off-diagonal cross-covariance penalty (Gaussian MI proxy) between z1, z2."""
        if z1.dim() == 3:
            z1 = z1.reshape(-1, z1.shape[-1])
            z2 = z2.reshape(-1, z2.shape[-1])
        ...
        std1 = z1c.std(dim=0, keepdim=True) + 1e-8
        std2 = z2c.std(dim=0, keepdim=True) + 1e-8
        z1n = z1c / std1
        z2n = z2c / std2

        # full cross-correlation matrix, shape (d1, d2)
        cross_corr = (z1n.T @ z2n) / N
        cross_corr = torch.clamp(cross_corr, -0.999, 0.999)

        # Gaussian MI applied elementwise, summed over ALL pairs (not just matched index)
        mi = -0.5 * torch.log(1 - cross_corr ** 2)
        return mi.sum()
    ...
    def loss(
        ...
        mi_loss_per_cell = torch.zeros(z1.shape[0], device=z1.device)
        if getattr(self, "mi_loss", False):
            mi_loss_val = self.get_decorrelation_loss(z1, z1r)
            if z1p is not None:
                mi_loss_val = mi_loss_val + self.get_decorrelation_loss(z1, z1p)
            if z1a is not None:
                mi_loss_val = mi_loss_val + self.get_decorrelation_loss(z1, z1a)
            mi_loss_per_cell = mi_loss_per_cell + mi_loss_val

        mi_weight = getattr(self, "mi_weight", 0.5)
        loss = torch.mean(
            reconst_loss_gene
        ...
        if getattr(self, "mi_loss", False):
            kl_local["mi_loss"] = mi_loss_per_cell
